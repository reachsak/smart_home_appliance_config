...
exports.handler = async (argv) => {
  let { username, password, file } = argv;
  ...
  if (!file && (!username || !password)) {
    log.error(
      `You must specify a username and password or a micloudlogin.json file containing the credentials!`
    );
    process.exit(0);
  }
  ...
  try {
    ...
    log.success(
      `Successfully logged in to MiCloud with username ${chalk.yellow.bold(
        username
      )}`
    );
  } catch (err) {
